...
# %%
# vae.load_state_dict(torch.load("vae_model_train_bin_test_e20_n50_241126234914.pth"))
# %%
vae.eval()
with torch.no_grad():    
...
# %%
# torch.save(recon_mat, f"vae_recon_e{epochs}_n{int(noise * 100)}_" + now +".pth")
...
for user, item, _, _ in train_user_item_pair:
# For user, item, _, _ in train_user_item_pair: # for experiment
    train_mat[user_idx[user], item_idx[item]] = 1.
